...
def verify_traversibility(map, neighbors=None):
    ...
    for location in neighbors:
        if(location[0]>=x_dims or location[1]>=y_dims):
            continue
        if(not(map[location[0]][location[1]]=='#') and ((map[location[0]][location[1]]==' ') or (map[location[0]][location[1]]=='S') or (map[location[0]][location[1]]=='G'))):
...
def astar(start, goal, map, movement_cost=10):
    start_node = Node(start, calculate_hueristic(start, goal, movement_cost))
...
def weighted_astar(start, goal, map, hueristic_weight=1, movement_cost=10):

    start_node = Node(start, calculate_weighted_hueristic(start, goal, hueristic_weight, movement_cost))
    goal_node = Node(goal, 0, None, -1)
    open_list = Open()
    closed_list = {}
    open_list.insert(start_node)
    ...
    while(len(open_list)>0):
        ...
        for neighbor in neighbors:
            ...
            neighbor_node = Node(neighbor, calculate_weighted_hueristic(neighbor, goal, hueristic_weight, movement_cost), current_node, current_node.g_cost+movement_cost)
            if(neighbor_node == goal_node):
                solution = retrace_path(start_node, neighbor_node)
                return solution
...
def anytime_weighted_astar(start, goal, map, hueristic_weight, movement_cost=10):
...
def anytime_weighted_astar_threaded(start, goal, map, hueristic_weight, movement_cost=10):
    ...
    closed_list = {}
    open_list.anytime_insert(start_node)
    flag = multiprocessing.Process(target=check_for_user_input)
    flag.daemon = True
    flag.start()
    incumbent_node = None
    incumbent_flag = 0
    print("\n---> press ENTER to terminate program <---\n\n Any suboptimal path, if found before termination, will be returned\n")

    while(len(open_list)>0 and flag.is_alive()):
